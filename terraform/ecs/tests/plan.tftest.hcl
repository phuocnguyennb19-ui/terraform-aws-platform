...
run "ecs_stack_for_a_mixed_environment" {
  command = plan
  variables {
    environment       = "dev"
  ...
  assert {
    condition     = length(module.ecs_service) == 1 && keys(module.ecs_service)[0] == "api" && length(module.alb) == 1
    error_message = "the ecs stack builds only the services placed on ecs"
  }
}
