# The developer contract on platform ecs: config.yaml → validated, policy-checked, deterministic module inputs.
...
run "dev_config_maps_to_module_inputs" {
  ...
  assert {
    condition     = local.enabled.ecs && !local.enabled.eks && length(local.release_specs) == 0
    error_message = "platform.type ecs builds ECS services and no cluster or Helm release."
  }
...
run "rejects_missing_platform" {
  ...
  variables {
    config_path = "tests/fixtures/missing_platform.yaml"
  }
...
run "rejects_unknown_platform" {
  ...
  variables {
    config_path = "tests/fixtures/unknown_platform.yaml"
  }

  expect_failures = [terraform_data.config]
}

run "rejects_eks_block_on_ecs" {
  command = plan
  plan_options {
    target = [terraform_data.config]
  }
  ...
  variables {
    config_path = "tests/fixtures/ecs_with_eks_block.yaml"
  }
